...
| `sessions.create` | `POST` | `/v1/sessions` | None | `{name, agent?, vcpus?, memory_mib?, allowed_hosts?, outbound_policy?, runtime_port?, background?}` | Section 6.2 session object | **201** |
...
| R-002-24 | MUST | WHEN `sessions.create` is invoked with `background` omitted or `false`, it SHALL preserve synchronous wire behavior, SHALL receive a non-`creating` session in the HTTP 201 response, and SHALL perform no readiness polling. | G-002-01, G-002-04 |
...
| R-002-26 | MUST | WHEN `sessions.create` is invoked with `background:true` and HTTP 201 returns a session whose status is `creating`, the consumer SHALL poll `sessions.get` for the returned session while the status remains `creating` before treating the machine as ready. | G-002-01, G-002-04 |
| R-002-27 | MUST | WHEN an SDK create request is encoded, the shared contract SHALL accept `background` only as an optional boolean, SHALL preserve explicit `true` and `false`, SHALL NOT infer a wire default when it is omitted, and SHALL continue to prohibit `terminal`, `api_key`, `token_saving`, and `capture_tool_io`. | G-002-01, G-002-04 |
...
| TC-002-16 | SDK create fixtures with `background` omitted and explicitly `false`, plus mutations adding each console-only create member or an automatic poll | `sessions.create` completes | The optional field is omitted or preserved as `false`, HTTP 201 returns a non-`creating` session once, no poll occurs, and every console-only field or poll mutation fails | R-002-24, R-002-27 |
| TC-002-21 | An SDK create fixture with `background:true` whose HTTP 201 session is `creating` | `sessions.create` is processed | The request preserves `background:true`, and the consumer polls `sessions.get` for the returned ID while the status remains `creating` before treating the machine as ready | R-002-26, R-002-27 |
...
| G-002-01, G-002-04 | R-002-24 | MUST | D-002-24 Synchronous create compatibility | T-002-24 Preserve omitted/false synchronous behavior | TC-002-16 | Planned / 1.3 |
| G-002-01, G-002-04 | R-002-26 | MUST | D-002-26 Background create readiness | T-002-26 Poll creating sessions by returned ID | TC-002-21 | Planned / 1.3 |
| G-002-01, G-002-04 | R-002-27 | MUST | D-002-27 Optional background wire field | T-002-27 Preserve booleans and reject console-only fields | TC-002-16, TC-002-21 | Planned / 1.3 |
